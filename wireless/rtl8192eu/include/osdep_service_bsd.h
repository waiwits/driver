...
#define LIST_CONTAINOR(ptr, type, member) ((type *)((char *)(ptr) - (SIZE_T)(&((type *)0)->member)))
...
struct sk_buff {
	...
	u8 pkt_type:3, fclone:2, ipvs_property:1, peeked:1, nf_trace:1;
	...
	u8 ndisc_nodetype:2, deliver_no_wcard:1;
#else
...
#define module_init(initfn) SYSINIT(mod_init_ ## initfn, SI_SUB_KLD, SI_ORDER_FIRST, module_init_exit_wrapper, initfn)

#define module_exit(exitfn) SYSUNINIT(mod_exit_ ## exitfn, SI_SUB_KLD, SI_ORDER_ANY, module_init_exit_wrapper, exitfn)
...
int rtw_usb_control_msg(struct usb_device *dev, struct usb_host_endpoint *uhe, uint8_t request, uint8_t requesttype, uint16_t value, uint16_t index, void *data, uint16_t size, usb_timeout_t timeout);
int rtw_usb_set_interface(struct usb_device *dev, uint8_t iface_no, uint8_t alt_index);
int rtw_usb_setup_endpoint(struct usb_device *dev, struct usb_host_endpoint *uhe, usb_size_t bufsize);
...
void rtw_usb_buffer_free(struct usb_device *dev, usb_size_t size, void *addr, uint8_t dma_addr);
...
void rtw_usb_fill_bulk_urb(struct urb *urb, struct usb_device *udev, struct usb_host_endpoint *uhe, void *buf, int length, usb_complete_t callback, void *arg);
int rtw_usb_bulk_msg(struct usb_device *udev, struct usb_host_endpoint *uhe, void *data, int len, uint16_t *pactlen, usb_timeout_t timeout);

...
//#endif
	...
	entry = create_proc_read_entry("mac_reg_dump1", S_IFREG | S_IRUGO, dir_dev, proc_get_mac_reg_dump1, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("mac_reg_dump2", S_IFREG | S_IRUGO, dir_dev, proc_get_mac_reg_dump2, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("mac_reg_dump3", S_IFREG | S_IRUGO, dir_dev, proc_get_mac_reg_dump3, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("bb_reg_dump1", S_IFREG | S_IRUGO, dir_dev, proc_get_bb_reg_dump1, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("bb_reg_dump2", S_IFREG | S_IRUGO, dir_dev, proc_get_bb_reg_dump2, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("bb_reg_dump3", S_IFREG | S_IRUGO, dir_dev, proc_get_bb_reg_dump3, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("rf_reg_dump1", S_IFREG | S_IRUGO, dir_dev, proc_get_rf_reg_dump1, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("rf_reg_dump2", S_IFREG | S_IRUGO, dir_dev, proc_get_rf_reg_dump2, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	{
		entry = create_proc_read_entry("rf_reg_dump3", S_IFREG | S_IRUGO, dir_dev, proc_get_rf_reg_dump3, dev);
		if (!entry)
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
		...
		entry = create_proc_read_entry("rf_reg_dump4", S_IFREG | S_IRUGO, dir_dev, proc_get_rf_reg_dump4, dev);
		if (!entry)
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
...
#ifdef CONFIG_80211N_HT
	entry = create_proc_read_entry("ht_enable", S_IFREG | S_IRUGO, dir_dev, proc_get_ht_enable, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("bw_mode", S_IFREG | S_IRUGO, dir_dev, proc_get_bw_mode, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("rx_stbc", S_IFREG | S_IRUGO, dir_dev, proc_get_rx_stbc, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("path_rssi", S_IFREG | S_IRUGO, dir_dev, proc_get_two_path_rssi, dev);
...
#ifdef CONFIG_BT_COEXIST
	entry = create_proc_read_entry("btcoex_dbg", S_IFREG | S_IRUGO, dir_dev, proc_get_btcoex_dbg, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	entry = create_proc_read_entry("sreset", S_IFREG | S_IRUGO, dir_dev, proc_get_sreset, dev);
	if (!entry)
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
...
//#endif
		if (TargetNetdev)
		{
			DBG_871X("Force onboard module driver to disappear!!!\n");
...
_func_enter_;
	...
	{
		RT_TRACE(_module_os_intfs_c_,_drv_err_,("\n Cannot init init_wifi_display_info\n"));
	}
  #endif
#endif /* CONFIG_P2P */
	...
	{
		DBG_871X("Cannot rtw_init_tdls_info\n");
		ret8 = _FAIL;
		goto exit;
	}
#endif //CONFIG_TDLS
	...
	{
		DBG_871X("Cannot _rtw_init_xmit_priv\n");
		ret8 = _FAIL;
		goto exit;
	}
	...
	{
		DBG_871X("Cannot _rtw_init_recv_priv\n");
		ret8 = _FAIL;
		goto exit;
	}
	...
	{
		DBG_871X("Cannot _rtw_init_sta_priv\n");
		ret8 = _FAIL;
		goto exit;
	}
	...
	{
		DBG_871X("%s: initialize MP private data failed!\n", __func__);
	}
#endif
...
//	pnetdev->stop = netdev_vir_if_close;
	...
	||  ((mac[0]==0x0) && (mac[1]==0x0) && (mac[2]==0x0) &&  (mac[3]==0x0) && (mac[4]==0x0) &&(mac[5]==0x0)))
	{
		mac[0] = 0x00;
		mac[1] = 0xe0;
		mac[2] = 0x4c;
...
int _netdev_open(struct net_device *pnetdev)
{
	...
	if (padapter->bup == _FALSE)
	{
		...
		if (status ==_FAIL)
		{
			RT_TRACE(_module_os_intfs_c_,_drv_err_,("rtl871x_hal_init(): Cannot init h/w!\n"));
...
int  ips_netdrv_open(_adapter *padapter)
{
	...
	if (status ==_FAIL)
	{
		RT_TRACE(_module_os_intfs_c_,_drv_err_,("ips_netdrv_open(): Cannot init h/w!\n"));
		goto netdev_open_error;
	}
	if (padapter->intf_start)
	{
...
int rtw_ips_pwr_up(_adapter *padapter)
{
	...
	rtw_led_control(padapter, LED_CTL_NO_LINK);
 	DBG_871X("<===  rtw_ips_pwr_up.............. in %d ms\n", rtw_get_passing_time_ms(start_time));
	return result;
}
...
void rtw_ips_pwr_down(_adapter *padapter)
{
	...
	padapter->bCardDisableWOHSM = _FALSE;
	DBG_871X("<=== rtw_ips_pwr_down..................... in %d ms\n", rtw_get_passing_time_ms(start_time));
}
#endif

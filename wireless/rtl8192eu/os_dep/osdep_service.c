...
inline u8* dbg_rtw_vmalloc(u32 sz, const char *func, int line)
{
	...
	rtw_update_mem_stat(p ? MEM_STAT_VIR_ALLOC_SUCCESS : MEM_STAT_VIR_ALLOC_FAIL, sz);
	return p;
}
...
inline void dbg_rtw_vmfree(u8 *pbuf, u32 sz, const char *func, int line)
{
	...
	_rtw_vmfree((pbuf), (sz)); 
	rtw_update_mem_stat(MEM_STAT_VIR_FREE, sz);
}
...
inline u8* dbg_rtw_malloc(u32 sz, const char *func, int line) 
{
	...
	p = _rtw_malloc((sz));
	rtw_update_mem_stat(p ? MEM_STAT_PHY_ALLOC_SUCCESS : MEM_STAT_PHY_ALLOC_FAIL, sz);
	return p;
}
...
inline void dbg_rtw_mfree(u8 *pbuf, u32 sz, const char *func, int line)
{
	...
	_rtw_mfree((pbuf), (sz));
	rtw_update_mem_stat(MEM_STAT_PHY_FREE, sz);
}
#endif
